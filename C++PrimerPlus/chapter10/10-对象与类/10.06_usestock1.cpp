#include <iostream>
#include "stock10.h"
//功能：10.6 使用类
//时间：2020年11月28日09:48:20
// 

//int main()
//{
//	{
//		using std::cout;
//		cout << "Using constructors to create new objects\n";
//		Stock stock1("NanoSmart", 12, 20.0);
//		stock1.show();
//		//通过初始化方式为对象进行赋值，可能会产生临时变量，但也可能不会，（这与所用的编译器有关）
//		Stock stock2 = Stock("Boffo Objects", 2, 2.0);    
//		stock2.show();
//
//		cout << "Assigning stock1 to stock2:\n";
//		stock2 = stock1;
//		cout << "Listing stock1 to stock2:\n";
//		stock1.show();
//		stock2.show();
//
//		cout << "Using a constructor to reset an object\n";
//		//通过赋值方式为对象进行赋值，一定会产生临时对象，再把临时对象的值赋给前面的对象，（降低了运行效率）
//		//这种方式多调用了一次构造函数与析构函数（具体看下面的输出：第六十行的Bye, Nifty Foods!）
//		stock1 = Stock("Nifty Foods", 10, 50.0);
//		cout << "Revised stock1:\n";
//		stock1.show();
//		cout << "Done\n";
//	}
//
//	system("pause");
//	return 0;
//}



/*
在x86编译环境下，运行结果如下：（每一个指针占用了四个字节的大小）
Using constructors to create new objects
Constructor using NanoSmart called
Company: NanoSmart Shares: 12
 Share Price: $20.000 Total Worth: $240.000
 Total Worth: $240.00
Constructor using Boffo Objects called
Company: Boffo Objects Shares: 2
 Share Price: $2.000 Total Worth: $4.000
 Total Worth: $4.00
Assigning stock1 to stock2:
Listing stock1 to stock2:
Company: NanoSmart Shares: 12
 Share Price: $20.000 Total Worth: $240.000
 Total Worth: $240.00
Company: NanoSmart Shares: 12
 Share Price: $20.000 Total Worth: $240.000
 Total Worth: $240.00
Using a constructor to reset an object
Constructor using Nifty Foods called
Bye, Nifty Foods!
Revised stock1:
Company: Nifty Foods Shares: 10
 Share Price: $50.000 Total Worth: $500.000
 Total Worth: $500.00
Done
Bye, NanoSmart!
Bye, Nifty Foods!
请按任意键继续. . .
-------------------------------------------------------------------------
在x64编译环境下，运行结果如下：（每一个指针占用了八个字节的大小）
*/